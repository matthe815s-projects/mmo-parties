...
import dev.matthe815.mmoparties.forge.helpers.ArrayHelpers;
...
public class PartyCommand extends CommandBase {
	...
	public List<String> getTabCompletions(MinecraftServer server, ICommandSender sender, String[] args, BlockPos targetPos) {
		...
		if (args.length < 2) {
			if (args.length == 0) {
				...
				if (ConfigHolder.COMMON.debugMode)
	...
	public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
		...
		switch (args[0]) {
			case "tp":
				if (!ConfigHolder.COMMON.allowPartyTP)
			...
			case "debug":
				if (!ConfigHolder.COMMON.debugMode) return;
				CommandMessageHelper.SendInfo( player, ConfigHolder.COMMON.debugMode ? "Debug Mode has been enabled" : "Debug Mode has been disabled." );
				break;
