...
public class PartyList {
    ...
    public static void onRenderGameOverlay(MatrixStack stack) {
        updateCounter++;
        ...
        for (PartyMemberData data : MMOPartiesCommon.localParty.data.values()) {
            ...
            lastOffset += RenderMember(new UISpec(stack, 0, lastOffset), data, playerNumber, (MMOPartiesCommon.localParty.local_players.size() > 4 || ConfigHolder.CLIENT.useSimpleUI.get()));
            playerNumber++;
        }
    }
     ...
     * @return X and Y
     */
    public static UISpec GetAnchorOffset(MatrixStack stack)
    {
        switch (ConfigHolder.CLIENT.anchorPoint.get()) {
            case "top-left":
                return new UISpec(stack, 8, 0);

            case "bottom-left":
                renderAscending = true;
                return new UISpec(stack, 8, mc.getWindow().getGuiScaledWidth());

            case "top-right":
                return new UISpec(stack, mc.getWindow().getGuiScaledWidth() - 5, 0);

            case "bottom-right":
                renderAscending = true;
                return new UISpec(stack, mc.getWindow().getGuiScaledWidth(), mc.getWindow().getGuiScaledWidth() - 20);
    ...
    public static int DrawNuggetBarCompact(UISpec UI, float current) {
        ...
        Renderer.drawString(new UISpec(UI.stack, UI.x + 12, UI.y), String.format("%s", Math.floor(current)));
        return 6;
    }
    ...
    public interface NuggetBar {
        int Render(MatrixStack stack, BuilderData data, int xOffset, int yOffset, boolean compact);
    }
    ...
    static int RenderMember(UISpec UI, PartyMemberData data, int playerNumber, boolean compact) {
        ...
        UISpec defaultOffset = GetAnchorOffset(UI.stack);
        ...
        if (compact) {
            ...
            nuggetBars[1].Render(UI.stack, data.additionalData[1], posX, ((defaultOffset.y - 10) + yOffset), true);
            nuggetBars[2].Render(UI.stack, data.additionalData[2], posX + 30 + (4 * data.name.length()), ((defaultOffset.y - 10) + yOffset), true);
        ...
        for (int i=0; i < nuggetBars.length; i++) {
            ...
            int offset = nuggetBars[i].Render(UI.stack, data.additionalData[i], posX, (defaultOffset.y + rowOffset) + yOffset, false);
    ...
    public static int DrawNuggetBar(UISpec UI, float current, float max) {
        ...
        if (ConfigHolder.CLIENT.numbersAsPercentage.get()) {
            ...
            if (!Objects.equals(ConfigHolder.CLIENT.extraNumberType, "additional")) current = (current / max) * 20;
    ...
    static void RenderExtraHealth(UISpec UI, float max, int extraHearts)
    {
        ...
        UISpec remainderSpec = new UISpec(UI.stack, remainderX, UI.y);
    ...
    {
        // Draw background
        Renderer.drawHalvedLayeredSprite(new UISpec(UI.stack, UI.texture, UI.textureHalf, UI.textureBack, x, y,9, 9), current, dropletHalf);
    }

    public static int DrawText(String text, UISpec location)
    {
        Minecraft.getInstance().font.draw(location.stack, text, location.x, location.y, 0xFFFFFF);
        return 8;
    }
