...
public class PartyList {
    ...
    public static void onRenderGameOverlay(Gui gui) {
        updateCounter++;
        ...
        for (PartyMemberData data : MMOPartiesCommon.localParty.data.values()) {
            ...
            if (ConfigHolder.CLIENT.hideSelf && data.name.equals(mc.player.getName())) continue;

            // Render a new player and track the additional offset for the next player.
            lastOffset += RenderMember(new UISpec(0, lastOffset), data, playerNumber, (MMOPartiesCommon.localParty.local_players.size() > 4 || ConfigHolder.CLIENT.useSimpleUI));
            playerNumber++;
        }
    }
    ...
    public static UISpec GetAnchorOffset()
    {
        ...
        switch (ConfigHolder.CLIENT.anchorPoint) {
            case "top-left":
            ...
            case "bottom-right":
                renderAscending = true;
                return new UISpec(window.getScaledHeight(), window.getScaledWidth() - 20);
    ...
    public static int DrawNuggetBarCompact(UISpec UI, float current) {
        ...
        Renderer.drawString(new UISpec(UI.x + 12, UI.y), String.format("%s", Math.floor(current)));
        return 6;
    }
    ...
    static int RenderMember(UISpec UI, PartyMemberData data, int playerNumber, boolean compact) {
        ...
        int configOffsetY = ConfigHolder.CLIENT.uiYOffset;
    ...
    public static int DrawNuggetBar(UISpec UI, float current, float max) {
        ...
        float maxLength = ConfigHolder.CLIENT.numbersAsPercentage ? 20 : max;

        // Normalize hearts into a value of 20 if the config is enabled.
        if (ConfigHolder.CLIENT.numbersAsPercentage) {
    ...
    static void RenderExtraHealth(UISpec UI, float max, int extraHearts)
    {
        ...
        UISpec remainderSpec = new UISpec(remainderX, UI.y);

        switch (ConfigHolder.CLIENT.extraNumberType) {
    ...
    {
        // Draw background
        Renderer.drawHalvedLayeredSprite(new UISpec(UI.texture, UI.textureHalf, UI.textureBack, x, y,9, 9), current, dropletHalf);
    }

    public static int DrawText(String text, UISpec location)
    {
        Minecraft.getMinecraft().fontRenderer.drawString(text, location.x, location.y, 0xFFFFFF);
        return 8;
    }
