...
{	
	...
	public void Invite ( EntityPlayer invoker, EntityPlayer player ) {
		...
		if ( invoker == player && !ConfigHolder.COMMON.debugMode )
		...
		if ( ( targetPlayer.InParty () || targetPlayer.partyInvite != null ) && !ConfigHolder.COMMON.debugMode ) // Players already in a party may not be invited.
	...
	public void Leave (EntityPlayer player)
	{
		...
		Broadcast( new TextComponentTranslation( "rpgparties.message.party.player.left", player.getName().toString() ) );
		...
		if (players.size() == 1 && !ConfigHolder.COMMON.autoAssignParties) Disband();
	...
	public void SendUpdate()
	{
		...
		for (EntityPlayer partyPlayer : players) {
			playerNames[i] = partyPlayer.getName();
			i++;
		}

		for (EntityPlayer partyPlayer : players) {
			if (!(partyPlayer instanceof EntityPlayerMP)) return;
			MMOParties.network.sendTo(new MessageUpdateParty(String.join(",", playerNames)), (EntityPlayerMP) partyPlayer);
		}
	}
	...
	public void SendPartyMemberData(EntityPlayer member, boolean bypassLimit, boolean remove)
	{
		if (IsDataDifferent(member) || bypassLimit)
		{
			PartyPacketDataBuilder builder = new PartyPacketDataBuilder ()
				...
				.SetLeader(this.leader.getName().equals(member.getName()))
				.SetAbsorption(member.getAbsorptionAmount())
				.SetHunger(member.getFoodStats().getFoodLevel());

			this.data.put(member.getName(), new PartyMemberData(builder));
	...
	{
		if (!this.data.containsKey(player.getName()) || this.data.get(player.getName()).IsDifferent(player))
			return true;
